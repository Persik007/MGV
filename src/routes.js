...
function handleUpload(req, res, sessions) {
  ...
  if (!contentType.includes('multipart/form-data')) {
    res.writeHead(400); res.end(JSON.stringify({ error: 'Expected multipart' })); return;
  }
  ...
  req.on('data', function(chunk) {
    ...
    if (totalSize > MAX_FILE_SIZE) {
      req.destroy();
      res.writeHead(413); res.end(JSON.stringify({ error: 'File too large (max 50MB)' }));
      return;
    }
    chunks.push(chunk);
  });
...
function handle(req, res, sessions) {
  ...
  if (url === '/api/stats' && req.method === 'GET') {
    ...
    if (!sessions.has(user)) { res.writeHead(401); res.end('{}'); return; }
    var u = db.getUser(user);
    if (!u || u.role !== 'admin') { res.writeHead(403); res.end('{}'); return; }
