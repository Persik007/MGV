        ...
        ws.on('message', function(raw) {
            ...
            if (msg.type === 'auth') {
                ...
                if (sessions.has(name)) {
                    try { sessions.get(name).send(JSON.stringify({ type: 'kicked', reason: 'Вход с другого устройства' }));
                        sessions.get(name).close(); } catch (e) {}
                }
                myName = name;
            ...
            if (msg.type === 'get-history') {
                ...
                var room = String(msg.room || '');
                var msgId = String(msg.msgId || '');
                var emoji = String(msg.emoji || '').slice(0, 8);
                ...
                if (sessions.has(target)) {
                    try { sessions.get(target).send(JSON.stringify({ type: 'kicked', reason: 'Вы заблокированы' }));
                        sessions.get(target).close(); } catch (e) {}
                }
